...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    @IBAction func refreshApps(_ sender: Any) {
        ...
        NSLog(running[3].localizedName!)
        NSWorkspace.shared.launchApplicationAtURL(running[3].bundleURL, configuration: <#[NSWorkspace.LaunchConfigurationKey : Any]#>)
    ...
    @IBAction func startCycling(_ sender: Any) {
        cycleTimer = Timer.scheduledTimer(timeInterval: 10.0, target: self, selector: #selector(AppDelegate.cycleApp), userInfo: nil, repeats: true)
    }
